...
{
    sealed class KvsJsonOutputFormatter : NewtonsoftJsonOutputFormatter
    {
        private readonly IReadOnlyDictionary<ApiVersion, IOuputSerializer<IEnumerable<KeyValue>>> _serializers;

        public KvsJsonOutputFormatter(
            JsonSerializerSettings settings,
            ArrayPool<char> charPool,
            MvcOptions mvcOptions,
            params IOuputSerializer<IEnumerable<KeyValue>>[] serializers)
        ...
        protected override bool CanWriteType(Type type)
        {
            //
            // Supported types 
            if (typeof(IEnumerable<KeyValue>).IsAssignableFrom(type))
            {
        ...
        public override async Task WriteResponseBodyAsync(OutputFormatterWriteContext ctx, Encoding selectedEncoding)
        {
            ...
            //
            // Get serializer
            IOuputSerializer<IEnumerable<KeyValue>> serializer = GetSerializer(ctx.HttpContext);
            ...
            {
                await serializer.WriteContent(jw, (IEnumerable<KeyValue>)ctx.Object, (long)fields);
        ...
        {
            IPage page = context.Object as IPage;

            //
            // Handle conditional request
            if (context.HttpContext.Request.IsRead() &&
                context.HttpContext.Request.TryEvaluatePreconditionStatusCode(page?.Etag, out int statusCode))
            {
            ...
            IEnumerable<KeyValue> kvs = (IEnumerable<KeyValue>)context.Object;
            ...
            //
            // Get serializer
            IOuputSerializer<IEnumerable<KeyValue>> serializer = GetSerializer(context.HttpContext);

            //
            // Write headers
            serializer.WriteResponseHeaders(context.HttpContext.Response, kvs);
        }

        private IOuputSerializer<IEnumerable<KeyValue>> GetSerializer(HttpContext context)
        {
            ...
            if (apiVersion == null ||
                !_serializers.TryGetValue(apiVersion, out IOuputSerializer<IEnumerable<KeyValue>> serializer))
            {
